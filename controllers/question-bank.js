  ...
  .controller('questionBankController', function ($scope, $http, $interval, $cookies, $sce, $timeout) {
    ...
    $scope.getPageNumbers = function () {
      ...
      if (endPage - startPage < 4) {
        if (startPage === 1) {
          endPage = Math.min($scope.totalPages, startPage + 4);
    ...
    $scope.getStartIndex = function () {
      if ($scope.summaryTileData.total == 0) return 0;
    ...
    $scope.getEndIndex = function () {
      ...
      var total = parseInt($scope.summaryTileData.total);
      if (localStorage.getItem("filterAppliedForQuestionBankListing") == "UNVERIFIED")
        total = parseInt($scope.summaryTileData.totalUnverified);
      else if (localStorage.getItem("filterAppliedForQuestionBankListing") == "CHALLENGED")
        total = parseInt($scope.summaryTileData.totalChallenged);
    ...
    $scope.refreshList = function () {
      if (localStorage.getItem("filterAppliedForQuestionBankListing") != "" && localStorage.getItem("filterAppliedForQuestionBankListing") != null)
    ...
    $scope.filterQuestions = function () {
      $scope.currentPage = 1;
      $scope.filterAllQuestions($scope.currentPage, $scope.maxResultsShown, $scope.questionBankFilterApplied || 'ALL');
    }
    ...
    //Default Listing
    $scope.currentPage = localStorage.getItem("currentPageQuestionBankListing") ? parseInt(localStorage.getItem("currentPageQuestionBankListing")) : 1;
    $scope.totalPages = Math.ceil($scope.summaryTileData.total / $scope.maxResultsShown);
